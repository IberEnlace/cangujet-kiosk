export async function loadAlertData(client:any, branchId:string, eventId:string, type:string){
  ...
  return{title:type.replaceAll("_"," ").replace(/\b\w/g,(c:string)=>c.toUpperCase()),summary:"cangujet detected an operational condition that may require attention.",rows,action:String(payload.suggested_action??"Review the affected workflow in cangujet Admin."),severity:event.severity};
}
