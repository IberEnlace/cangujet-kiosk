...
export default function DeviceSetup({ onConfigured, onDeviceInfo }: { onConfigured: () => void; onDeviceInfo: () => void }) {
  ...
  return <main dir="ltr" className={`device-setup-page ${workspaceMode ? "device-setup-page--workspace" : ""} min-h-[100dvh] bg-[#080b08] px-5 py-8 font-['DM_Sans'] text-white sm:px-10`}>
    <div className={`device-setup-shell mx-auto flex min-h-[calc(100dvh-4rem)] w-full flex-col ${workspaceMode ? "max-w-[1180px]" : "max-w-2xl"}`}>
      {!workspaceMode && <header className="device-setup-header flex items-center justify-between"><MorrowLogo variant="full" priority className="h-auto w-44" /><div className="device-setup-languages flex gap-1">{SUPPORTED_LANGUAGE_CODES.map(item => <button type="button" key={item} onClick={() => setLanguage(item)} className={`min-h-10 rounded-xl px-3 text-xs font-bold uppercase ${language === item ? "bg-[#D7FB69] text-[#17200f]" : "bg-white/5 text-white/45"}`}>{item}</button>)}</div></header>}
      <section className={workspaceMode ? "device-workspace-stage" : "my-auto rounded-[2rem] border border-white/10 bg-white/[.035] p-[clamp(1.5rem,5vw,3rem)] shadow-2xl"}>
        {status === "configured" && config ? <ConfiguredState config={config} text={text.connected} selectedType={selectedType} /> : verifiedKey ? <DeviceWorkspaceStage verification={verifiedKey} selectedType={selectedType} busy={initializationStatus === "registering"} error={displayedError} onBack={resetVerification} onActivate={activate} /> : <>
          ...
          <form onSubmit={submit} className="mt-8"><label className="text-sm font-semibold text-white/70">{text.label}<div className="relative mt-2"><input type={visible ? "text" : "password"} value={secretKey} onChange={event => { const value = event.target.value; setSecretKey(/^\s*morrow/i.test(value) ? normalizeDeviceActivationKey(value) : value); }} autoCapitalize="characters" autoCorrect="off" spellCheck={false} autoComplete="off" placeholder={text.placeholder} aria-describedby="device-key-help" className="min-h-16 w-full rounded-2xl border border-white/12 bg-black/25 px-5 pe-28 font-mono text-base uppercase tracking-[.08em] outline-none focus:border-[#D7FB69]/60 sm:text-lg" /><button type="button" onClick={() => setVisible(value => !value)} className="absolute end-3 top-3 flex min-h-10 items-center gap-2 rounded-xl px-3 text-xs text-white/55">{visible ? <EyeOff size={17}/> : <Eye size={17}/>} {visible ? text.hide : text.show}</button></div><span id="device-key-help" className="mt-2 block text-xs font-normal text-white/35">Paste is supported. The key is removed after activation.</span></label>
            ...
            <button type="submit" disabled={initializationStatus === "registering" || status === "connecting"} className="mt-6 flex min-h-16 w-full items-center justify-center gap-3 rounded-2xl bg-[#D7FB69] text-lg font-bold text-[#17200f] disabled:opacity-50">{status === "connecting" ? <Loader2 className="animate-spin"/> : error ? <RotateCcw/> : null}{error ? text.retry : "Verify key"}</button>
          </form>
        </>}
      </section>
      <footer className={`device-setup-footer flex flex-wrap justify-center gap-2 text-xs text-white/40 ${workspaceMode ? "device-setup-footer--workspace" : ""}`}>{config && <button onClick={onDeviceInfo} className="flex min-h-11 items-center gap-2 rounded-xl bg-white/5 px-4"><Info size={15}/>{text.info}</button>}<button onClick={() => { void clearDeviceConfiguration(); setSecretKey(""); resetVerification(); }} className="flex min-h-11 items-center gap-2 rounded-xl bg-white/5 px-4"><Trash2 size={15}/>{text.clear}</button></footer>
    </div>
  </main>;
}
...
function ConfiguredState({ config, text, selectedType }: { config: NonNullable<ReturnType<typeof useDevice>["config"]>; text: string; selectedType: BootstrapDeviceType | null }) {
  ...
  return <motion.div layoutId={workspaceLayoutId(workspace)} className="text-center"><CheckCircle2 className="mx-auto size-16 text-[#D7FB69]" /><h1 className="mt-6 text-3xl font-bold">{text}</h1><p className="mt-3 text-white/45">{config.kioskName} · {config.branchName}</p><div className="mx-auto mt-7 max-w-sm space-y-3 text-left">{setupSteps.map((step, index) => <div key={step} className="flex items-center gap-3 rounded-xl bg-white/[.035] px-4 py-3 text-sm text-white/70"><span className="grid size-6 place-items-center rounded-full bg-[#D7FB69]/15 text-xs font-bold text-[#D7FB69]">{index + 1}</span>{step}</div>)}</div></motion.div>;
}

function ErrorPanel({ error }: { error: readonly [string, string] }) {
  return <div role="alert" className="mt-4 rounded-2xl border border-red-400/20 bg-red-500/10 p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-red-300" size={20}/><div><strong>{error[0]}</strong><p className="mt-1 text-sm text-white/55">{error[1]}</p></div></div></div>;
}
