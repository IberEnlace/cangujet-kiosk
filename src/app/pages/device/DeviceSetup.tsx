...
import { isDeviceSecretKey } from "../../../shared/deviceKey";

const copy = {
  en: { title: "Set up this kiosk", description: "Enter the device secret key provided for this kiosk.", label: "Device secret key", placeholder: "Enter secret key", connect: "Connect device", show: "Show key", hide: "Hide key", retry: "Retry", clear: "Clear setup", info: "Show device information", connected: "Device connected", invalid_request: ["Invalid setup request", "Enter the complete device key exactly as provided."], invalid_key: ["Invalid device key", "Check the key and try again."], network_error: ["Unable to connect", "Check the kiosk internet connection and try again."], timeout: ["Connection timed out", "The device service did not respond. Try again."], disabled: ["This kiosk is disabled", "Contact your administrator."], expired: ["This device key has expired", "Ask your administrator for a new key."], conflict: ["Device session conflict", "Clear setup and try the key again."], server_error: ["Device service unavailable", "Check the server configuration and try again."], protocol_error: ["Unexpected server response", "Retry or contact your administrator."], configuration_error: ["Setup could not be completed", "The device configuration is incomplete or invalid."] },
  tr: { title: "Bu kiosku kurun", description: "Bu kiosk için verilen cihaz gizli anahtarını girin.", label: "Cihaz gizli anahtarı", placeholder: "Gizli anahtarı girin", connect: "Cihazı bağla", show: "Anahtarı göster", hide: "Anahtarı gizle", retry: "Tekrar dene", clear: "Kurulumu temizle", info: "Cihaz bilgilerini göster", connected: "Cihaz bağlandı", invalid_request: ["Geçersiz kurulum isteği", "Verilen cihaz anahtarının tamamını aynen girin."], invalid_key: ["Geçersiz cihaz anahtarı", "Anahtarı kontrol edip tekrar deneyin."], network_error: ["Bağlantı kurulamadı", "Kioskun internet bağlantısını kontrol edip tekrar deneyin."], timeout: ["Bağlantı zaman aşımına uğradı", "Cihaz hizmeti yanıt vermedi. Tekrar deneyin."], disabled: ["Bu kiosk devre dışı", "Yöneticinizle iletişime geçin."], expired: ["Cihaz anahtarının süresi dolmuş", "Yöneticinizden yeni bir anahtar isteyin."], conflict: ["Cihaz oturumu çakışması", "Kurulumu temizleyip yeniden deneyin."], server_error: ["Cihaz hizmeti kullanılamıyor", "Sunucu yapılandırmasını kontrol edip yeniden deneyin."], protocol_error: ["Beklenmeyen sunucu yanıtı", "Yeniden deneyin veya yöneticinizle iletişime geçin."], configuration_error: ["Kurulum tamamlanamadı", "Cihaz yapılandırması eksik veya geçersiz."] },
} as const;

type SetupLanguage = SupportedLanguage;
type ErrorStatus = Exclude<DeviceStatus, "checking" | "unconfigured" | "connecting" | "configured">;
const errorStatuses: ErrorStatus[] = ["invalid_request", "invalid_key", "network_error", "timeout", "disabled", "expired", "conflict", "server_error", "protocol_error", "configuration_error"];
...
export default function DeviceSetup({ onConfigured, onDeviceInfo }: { onConfigured: () => void; onDeviceInfo: () => void }) {
  ...
  const displayedError = validationError ? ["Invalid key format", validationError] as const : error;
  ...
  const submit = async (event: FormEvent) => {
    ...
    diagnostic("setup_submit_started");
    const trimmed = secretKey.trim();
    if (!isDeviceSecretKey(trimmed)) {
      diagnostic("setup_validation_failed");
      setValidationError("Enter the complete mdk_ device key exactly as provided.");
      return;
    }
    setValidationError("");
...
function diagnostic(event: "setup_submit_started" | "setup_validation_failed") {
  if (import.meta.env?.DEV) console.info("[MORROW device]", { event });
}
