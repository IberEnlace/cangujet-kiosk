...
export default function AdminNotifications() {
  ...
  return <>
    ...
    {testOpen && <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-[#1F1F1F]/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Send Test Notification"><div className="w-full max-w-xl rounded-2xl border border-[#ECECEC] bg-white p-5 text-[#1F1F1F] shadow-[0_24px_80px_rgba(31,31,31,.18)]"><h2 className="mb-5 text-lg font-bold">Send Test Notification</h2><Field label="Recipient Email" value={recipient} onChange={setRecipient}/><p className="mt-3 text-[11px] text-[#9CA3AF]">This sends a real cangujet test email through the configured provider.</p><div className="mt-5 flex justify-end gap-2"><button disabled={sending} onClick={() => setTestOpen(false)} className={`${button} border-[#ECECEC] bg-white text-[#1F1F1F] hover:bg-[#F8F9FA]`}>Cancel</button><button disabled={sending} onClick={() => void sendTest()} className={`${button} bg-[#C41E19] text-white hover:bg-[#A8161A]`}>{sending ? "Sending…" : "Send Test"}</button></div></div></div>}
  </>;
}
