...
const button = "admin-button rounded-xl px-4 py-2.5 text-sm font-bold focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C41E19]/55 disabled:cursor-not-allowed disabled:opacity-40";
...
export default function AdminNotifications() {
  ...
  return <>
    ...
    <div className="grid max-w-5xl gap-5 lg:grid-cols-2">
      ...
      <div className="flex flex-wrap gap-2 lg:col-span-2"><button disabled={saving || auth.isDemoAuth} onClick={() => void save()} className={`${button} bg-[#C41E19] text-[#FFFFFF]`}>{saving ? "Saving…" : "Save Notification Settings"}</button><button disabled={sending || auth.isDemoAuth} onClick={() => { setRecipient(settings.restaurantEmail); setTestOpen(true); }} className={`${button} border border-white/10 bg-white/5`}>Send Test Notification</button><button disabled={reporting || auth.isDemoAuth} onClick={() => void sendReport()} className={`${button} border border-white/10 bg-white/5`}>{reporting ? "Generating…" : "Send Daily Report Now"}</button></div>
