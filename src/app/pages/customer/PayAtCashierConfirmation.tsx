...
export default function PayAtCashierConfirmation({ onReset }: { onReset: () => void }) {
  ...
  return <main dir={direction} className="min-h-[100dvh] bg-[#070a07] font-['Plus_Jakarta_Sans'] text-[#f4f5ef]">
    <div className="relative mx-auto flex min-h-[100dvh] w-full max-w-[780px] flex-col overflow-hidden px-6 py-7 sm:px-12 sm:py-10">
      ...
      <header className="relative flex items-center justify-between">
        ...
        <span className="rounded-full border border-[#C41E19]/20 bg-[#C41E19]/8 px-4 py-2 text-[11px] font-black uppercase tracking-[.18em] text-[#C41E19]">Pay at Cashier</span>
      </header>
      ...
      {phase === "printing" ? <section className="relative flex flex-1 flex-col items-center justify-center py-8 text-center" aria-live="polite">
        ...
        <div className="mt-1 flex items-center gap-3 text-sm font-bold text-white/55"><span className="size-2 animate-pulse rounded-full bg-[#C41E19]" />Printing…</div>
        <div className="mt-5 h-1.5 w-64 overflow-hidden rounded-full bg-white/8"><div className="h-full origin-left animate-[pulse_1.2s_ease-in-out_infinite] rounded-full bg-[#C41E19]" /></div>
      </section> : <section className="relative flex flex-1 flex-col items-center py-8 text-center" aria-live="polite">
        <div className="grid size-20 place-items-center rounded-full bg-[#C41E19] text-[#FFFFFF] shadow-[0_0_55px_rgba(215,251,105,.2)]"><CheckCircle2 size={43} strokeWidth={2.5} /></div>
        ...
        <div className="mt-7 grid w-full gap-5 sm:grid-cols-[minmax(0,1fr)_240px] sm:text-left">
          <div className="rounded-[28px] border border-[#C41E19]/18 bg-[#C41E19]/[.055] p-6">
        ...
        <div role="status" className={`mt-5 flex min-h-12 items-center gap-2 rounded-2xl border px-4 text-sm font-bold ${printStatus === "printer_error" ? "border-amber-300/25 bg-amber-300/8 text-amber-100" : "border-[#C41E19]/20 bg-[#C41E19]/7 text-[#C41E19]"}`}>
      ...
      {phase === "ready" ? <footer className="relative border-t border-white/8 pt-5">
        <div className={`grid gap-3 ${printStatus === "printer_error" ? "sm:grid-cols-2" : ""}`}>
          ...
          <button type="button" onClick={finishSession} className="min-h-16 rounded-2xl bg-[#C41E19] px-6 text-lg font-black text-[#FFFFFF] shadow-[0_12px_34px_rgba(215,251,105,.14)] transition hover:bg-[#c9ed65] active:scale-[.99]">Finish</button>
        </div>
...
function TicketSummary({ snapshot, itemCount }: { snapshot: PayAtCashierConfirmationSnapshot; itemCount: number }) {
  return <article dir="ltr" className="rotate-[.5deg] bg-[#fffdf7] px-5 py-5 text-left font-mono text-[#FFFFFF] shadow-2xl shadow-black/30 [clip-path:polygon(0_0,100%_0,100%_96%,95%_100%,90%_96%,85%_100%,80%_96%,75%_100%,70%_96%,65%_100%,60%_96%,55%_100%,50%_96%,45%_100%,40%_96%,35%_100%,30%_96%,25%_100%,20%_96%,15%_100%,10%_96%,5%_100%,0_96%)]">
