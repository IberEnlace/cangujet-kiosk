...
export default function MenuCatalog({ onBack, onCheckout, onLanguage, onNori, onNoriChat }: MenuCatalogProps) {
  ...
  return (
    <main dir={direction} className="min-h-[100dvh] bg-[#050705] font-['Plus_Jakarta_Sans'] text-[#f8f8f3]">
      <div className="relative mx-auto min-h-[100dvh] w-full max-w-[900px] overflow-hidden bg-[#0b1009] shadow-2xl shadow-black/40">
        ...
        <header className="sticky top-0 z-30 flex h-[clamp(4.6rem,7vh,6rem)] items-center justify-between border-b border-white/10 bg-[#0b1009]/92 px-3 backdrop-blur-xl sm:px-5">
          ...
          <div className="flex items-center gap-2">{kiosk?.ai.enabled && <button type="button" onClick={onNori} className="hidden min-h-11 rounded-xl border border-[#C41E19]/25 bg-[#C41E19]/8 px-3 text-xs font-bold text-[#C41E19] min-[560px]:block">Ask Nori</button>}<button type="button" onClick={onLanguage} className="min-h-11 min-w-11 rounded-xl border border-white/10 bg-white/5 px-2 text-xs font-bold uppercase text-white/70 focus-visible:outline focus-visible:outline-4 focus-visible:outline-[#d7ff7a]">{language}</button><button type="button" onClick={onCheckout} aria-label={`Open cart with ${count} items`} className="relative grid size-12 place-items-center rounded-2xl bg-[#d7ff7a] text-[#FFFFFF] focus-visible:outline focus-visible:outline-4 focus-visible:outline-white"><ShoppingBag size={20} aria-hidden="true" />{count > 0 && <span className="absolute -end-1 -top-1 grid size-5 place-items-center rounded-full bg-[#ffb86c] text-[10px] font-bold text-[#34240e]">{count}</span>}</button></div>
        </header>

        <div className="grid min-h-[calc(100dvh-5rem)] grid-cols-[88px_minmax(0,1fr)] sm:grid-cols-[120px_minmax(0,1fr)]">
          <aside className="relative border-e border-white/10 bg-[#0e130c] p-2 pb-28"><p className="px-2 pb-2 pt-2 font-['Plus_Jakarta_Sans'] text-[8px] tracking-[.14em] text-white/35">MENU</p><nav className="space-y-2" aria-label="Menu categories">{menuCategories.map(item => <button type="button" key={item.id} onClick={() => chooseCategory(item.id)} aria-pressed={category === item.id} className={`flex min-h-[78px] w-full flex-col items-center justify-center rounded-2xl border px-1 py-2 transition active:scale-95 focus-visible:outline focus-visible:outline-3 focus-visible:outline-[#d7ff7a] ${category === item.id ? "border-[#d7ff7a]/50 bg-[#d7ff7a]/10 shadow-[0_5px_20px_rgba(215,255,122,.08)]" : "border-transparent hover:bg-white/5"}`}><span className={`grid size-11 place-items-center rounded-full text-xl ${category === item.id ? "bg-[#d7ff7a] text-[#FFFFFF]" : "bg-white/5 text-white/45"}`}>{item.image ? <img src={item.image} alt="" className="size-10 rounded-full object-cover" /> : item.icon || item.name.charAt(0)}</span><span className={`mt-1.5 text-[9px] leading-3 sm:text-[10px] ${category === item.id ? "font-bold text-[#d7ff7a]" : "text-white/45"}`}>{item.name}</span></button>)}</nav></aside>

          <section className="relative min-w-0 pb-28"><div className="p-3 sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-['Plus_Jakarta_Sans'] text-[8px] tracking-[.14em] text-[#d7ff7a] sm:text-[9px]">{view === "products" ? "CATEGORY SELECTED" : "CHOOSE A CATEGORY"}</p><h1 className="mt-1 text-[clamp(1.45rem,4vw,2.2rem)] font-semibold tracking-[-.04em] text-white">{view === "products" ? selectedCategory?.name : "What are you craving?"}</h1></div><button type="button" onClick={() => setView("categories")} className="shrink-0 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white/55">Categories</button></div>
...
function NoriBanner({ onOpenChat, isProcessing, sendMessage, language, voiceEnabled }: { onOpen: () => void; onOpenChat: () => void; isProcessing: boolean; sendMessage: (text: string) => Promise<NoriConversationReply | null>; language: SupportedLanguage; voiceEnabled: boolean }) {
  ...
  return <section aria-labelledby="nori-banner-title" className="relative mt-5 overflow-hidden rounded-[26px] border border-[#C41E19]/20 bg-[linear-gradient(135deg,rgba(215,251,105,.095),rgba(255,255,255,.025)_58%,rgba(215,251,105,.055))] p-4 shadow-[0_18px_45px_rgba(0,0,0,.2)] sm:p-5">
    ...
    <div className="relative flex flex-col gap-4 min-[680px]:flex-row min-[680px]:items-center min-[680px]:justify-between">
      <div className="min-w-0 flex-1">
        <p className="font-['Plus_Jakarta_Sans'] text-[9px] font-bold uppercase tracking-[.16em] text-[#C41E19]">Hi, I’m Nori! 👋</p>
        ...
        <div className="mt-3 flex flex-wrap gap-2" aria-label="Quick recommendation prompts">{quickPrompts.map(([title, request]) => <button key={title} type="button" disabled={busy} onClick={() => submitPrompt(request)} className="min-h-12 rounded-full border border-white/10 bg-white/[.045] px-3 text-[11px] font-semibold text-white/65 transition hover:border-[#C41E19]/35 hover:bg-[#C41E19]/8 hover:text-white active:scale-95 focus-visible:outline focus-visible:outline-3 focus-visible:outline-[#C41E19] disabled:opacity-40">{title}</button>)}</div>
        {error && <div role="alert" className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-amber-300/15 bg-amber-300/[.06] px-3 py-2 text-[11px] leading-4 text-amber-50/75"><span className="flex-1">{error}</span><button type="button" onClick={() => void listen()} className="flex min-h-12 items-center gap-1.5 rounded-lg px-3 font-bold text-[#C41E19] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#C41E19]"><RefreshCw size={14} aria-hidden="true" />Retry</button><button type="button" onClick={onOpenChat} className="min-h-12 rounded-lg px-3 font-bold text-white/75 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#C41E19]">Type instead</button></div>}
      </div>
  ...
  return <div className="mt-6 grid grid-cols-1 gap-x-4 gap-y-5 min-[560px]:grid-cols-2">{menu.categories.map(category => {
    ...
    return <button type="button" key={category.id} onClick={() => onChoose(category.id)} aria-pressed={isSelected} className="group relative h-[clamp(11rem,20vh,13.5rem)] overflow-visible rounded-[24px] text-start transition duration-300 ease-out hover:-translate-y-1.5 hover:scale-[1.03] active:scale-[.99] focus-visible:outline focus-visible:outline-4 focus-visible:outline-[#d7ff7a]">
      ...
      <span className="absolute inset-x-5 bottom-5 z-20">
        ...
        <span className="mt-2 block font-['Plus_Jakarta_Sans'] text-[9px] font-bold uppercase tracking-[.13em] text-[#d7ff7a]/80">{itemCount} Items</span>
      </span>
    </button>;
  })}</div>;
}
...
function ProductGrid({ products, currency, onAdd }: { products: NormalizedMenuProduct[]; currency: Intl.NumberFormat; onAdd: (product: NormalizedMenuProduct) => void }) {
  return <div className="mt-5 grid grid-cols-1 gap-3 min-[560px]:grid-cols-2">{products.map(product => {const badge=product.dietaryTags.includes("vegetarian")?"VEGETARIAN":undefined;return <article key={product.id} className="relative overflow-hidden rounded-[24px] border border-white/10 bg-white/[.045] p-3 shadow-[0_12px_30px_rgba(0,0,0,.18)]"><div className="relative grid aspect-square w-full place-items-center overflow-hidden rounded-[18px] bg-[radial-gradient(circle_at_40%_35%,rgba(215,255,122,.13),rgba(255,255,255,.025))]">{product.image ? <img src={product.image} alt={product.name} className="absolute inset-0 size-full object-contain p-2" /> : <span className="grid size-20 place-items-center rounded-full bg-[#d7ff7a] text-4xl font-black text-[#FFFFFF] shadow-xl shadow-[#d7ff7a]/10">{product.name.charAt(0)}</span>}{badge && <span className="absolute start-2 top-2 z-10 rounded-full bg-[#d7ff7a] px-2 py-1 font-['Plus_Jakarta_Sans'] text-[8px] font-bold tracking-wider text-[#FFFFFF]">{badge}</span>}</div><h2 className="mt-3 text-[15px] font-semibold leading-5 text-white">{product.name}</h2><p className="mt-1 min-h-8 text-[11px] leading-4 text-white/40">{product.description}</p><div className="mt-3 flex items-end justify-between"><div><b className="text-[#d7ff7a]">{currency.format(product.price)}</b><small className="block text-[10px] text-white/30">{product.calories} kcal</small></div><button type="button" onClick={() => onAdd(product)} aria-label={`Add ${product.name}`} className="grid size-12 place-items-center rounded-xl bg-[#d7ff7a] text-[#FFFFFF] transition active:scale-95 focus-visible:outline focus-visible:outline-4 focus-visible:outline-white"><Plus size={20} aria-hidden="true" /></button></div></article>})}</div>;
}
