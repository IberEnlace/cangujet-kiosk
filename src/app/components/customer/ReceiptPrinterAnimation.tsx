...
export default function ReceiptPrinterAnimation({ status }: { status: ReceiptPrintStatus }) {
  return <div className="confirmation-printer-visual relative mx-auto h-52 w-64" aria-hidden="true">
    ...
    <div className="absolute inset-x-0 bottom-0 h-28 rounded-[26px] border border-[#ECECEC] bg-[#FFFFFF] shadow-[0_12px_32px_rgba(31,31,31,.10)]"><div className="mx-auto mt-6 h-3.5 w-40 rounded-full bg-[#1F1F1F] shadow-inner" /><div className="mx-auto mt-5 flex w-44 items-center justify-between"><CangujetLogo variant="symbol" className="size-7 object-contain" alt="" /><span className={`grid size-7 place-items-center rounded-full ${status === "printed" ? "bg-[#C41E19] text-[#FFFFFF]" : "bg-[#F8F9FA] text-[#9CA3AF]"}`}>{status === "printed" && <Check size={16} />}</span></div></div>
  </div>;
}
