...
export interface CashierReceiptData { orderNumber: string; date: string; cashierName: string; restaurantName: string; branchName: string; registerName: string; currency: string; items: CashierReceiptItem[]; subtotal: number; taxRate: number; taxAmount: number; total: number; paymentMethod: "cash" | "card"; amountReceived?: number; change?: number }
...
function ReceiptItemRow({ item, money }: { item: CashierReceiptItem; money: Intl.NumberFormat }) {
...
export default function CashierReceipt({ receipt, mode }: CashierReceiptProps) {
  ...
  return <article id={mode==="print"?"cashier-print-receipt":undefined} className={`cashier-receipt font-mono text-[11px] leading-[1.4] [font-variant-numeric:tabular-nums] ${shell}`}>
    ...
    <section className="receipt-items mt-3 border-t border-dashed border-black/45 pt-1">{receipt.items.map(item=><ReceiptItemRow key={item.id} item={item} money={money}/>)}</section>
