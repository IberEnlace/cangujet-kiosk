...
test("the preparing transition is complete in English and Turkish", () => {
  assert.match(preparing, /title: "Preparing your device\.\.\."/);
  assert.match(preparing, /description: "This will only take a moment\."/);
  assert.match(preparing, /title: "Cihazınız hazırlanıyor\.\.\."/);
  assert.match(preparing, /description: "Bu işlem yalnızca kısa bir süre alacaktır\."/);
});
