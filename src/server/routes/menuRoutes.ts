...
menuRouter.get("/categories", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
  const { data, error } = await db.from("categories").select("*").order("display_order").order("name");
  if (error) return fail(response, 500, "Categories could not be loaded.");
...
menuRouter.put("/categories/:id", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
...
menuRouter.delete("/categories/:id", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
...
menuRouter.get("/products", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
  const { data, error } = await db.from("products").select("*").order("display_order").order("name");
  if (error) return fail(response, 500, "Products could not be loaded.");
  return response.json((data ?? []).map(mapProduct));
});
menuRouter.post("/products", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
...
menuRouter.put("/products/:id", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
...
menuRouter.delete("/products/:id", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
...
menuRouter.post("/menu-images", async (request, response) => {
  const db = client(request); if (!db) return fail(response, 503, "Database is not configured.");
