...
import { matchesFlavor } from "./noriRecommendationRanker";

export type NoriValidationReason = "over_budget" | "below_min_protein" | "above_max_calories" | "above_max_fat" | "above_max_sugar" | "below_min_fiber" | "above_max_sodium" | "above_max_carbohydrates" | "wrong_category" | "excluded_ingredient" | "preference_conflict" | "dietary_conflict" | "direct_allergen" | "unavailable";
...
export function validateNoriCandidate(product: AIFoodItem, constraints: NoriRequestConstraints): NoriValidationReason[] {
  ...
  if (constraints.excludedIngredients.some(value =>
    value === "spicy" ? product.spiceLevel > 0 || haystack.includes("spicy") : haystack.includes(value.toLowerCase()))) {
    reasons.push("excluded_ingredient");
  }
  if (constraints.preferredFlavors.length
    && !constraints.preferredFlavors.some(flavor => matchesFlavor(product, flavor))) reasons.push("preference_conflict");
